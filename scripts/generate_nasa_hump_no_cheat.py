...
def knn_idw_predict(train_xyz: np.ndarray, train_u: np.ndarray, query_xyz: np.ndarray, k: int, chunk_size: int) -> np.ndarray:
...
def main() -> int:
    ...
    try:
        ...
        if not train_cases:
            raise RuntimeError("No non-NASA training cases found.")
        ...
        prediction = knn_idw_predict(train_xyz, train_u, query_xyz, args.k, args.chunk_size)
    ...
    truth = knn_idw_predict(nasa_coords, nasa_u, query_xyz, 1, args.chunk_size)
